...
#Remove PCR Replicates and Adapter sequences from FASTQ's, followed by STAR alignment and the Splice-Break2 v3.0.0 pipeline
...
star="/usr/local/bin/STAR"  #250408changed
samtools="samtools"
SB="${SB_Path}/Splice-Break_v3.0.1/Splice-Break2_0423_star.sh"  #241221changed
# put own direct wrapper with sp
mouse_ref="/home/hrknkg/ref/GRCm39_star_mtonly" #250408changed
human_ref="/home/hrknkg/ref/GRCh38_star/combined_GRCh38_mt" #241224added
...
elif [ ${ref} == "--ref=mouse" ];then  #241221added for mouse
    refGenome=${mouse_ref}
    reference_name=NC_005089.1
    echo "refGenome: ${refGenome}" >> ${logDir}/temp.log
    echo "reference Name: ${reference_name}" >> ${logDir}/temp.log
elif [ ${ref} == "--ref=human" ];then  #241224added for mouse
    refGenome=${human_ref}
    reference_name=NC_012920.1
    echo "refGenome: ${refGenome}" >> ${logDir}/temp.log
    echo "reference Name: ${reference_name}" >> ${logDir}/temp.log
fi
...
if [ ${align} == "--align=no" ]||[ ${align} == '--align=n' ]||[ ${align} == '--align=No' ]||[ ${align} == '--align=N' ]||[ ${align} == '--align=NO' ]; then
   ...
   # get the filename as j
    ...
    if [ -d ${outputDir}/${j}_mapsplice* ] && [ -f ${outputDir}/${j}_mapsplice*/Large_Deletions_NC_012920.1_No-Position-Filter.txt ] ; then
        ...
        ${star} \
           --genomeDir ${refGenome} \
           --readFilesIn $READ_FILE_END \
           --outFileNamePrefix ${finalOutput}/ \
           --outSAMtype BAM SortedByCoordinate \
           --runThreadN 8 \
           2> ${finalOutput}/${j}_mapsplice.log  #241221changed
        if [ $? -gt 0 ]; then
           echo "#ERROR: Mapsplice Fail. Check logs">> ${outputDir}/${sampleName}_nohup.log
           exit
        fi
        mv -v ${finalOutput}/Aligned.sortedByCoord.out.bam ${finalOutput}/alignments.bam>> ${outputDir}/${sampleName}_nohup.log
        echo "--------------------------------------">> ${outputDir}/${sampleName}_nohup.log
        ...
        ${star} \  #241221changed
           ...
           --outSAMtype BAM SortedByCoordinate \
           --runThreadN 8 \
           2> ${finalOutput}/${j}_mapsplice.log
        if [ $? -gt 0 ]; then
           echo "#ERROR: Mapsplice Fail. Check logs">> ${outputDir}/${sampleName}_nohup.log
           exit
        fi
