...
my $star = $User_Preferences->{star}; #241204added
...
my $stindex = $User_Preferences->{stindex}; #241204added
...
if($nu_mt eq 'yes' && $enriched eq 'no'){
  ...
  if($dna eq 'no'){system("$star --runThreadN $threads --genomeDir $stindex --readFilesIn $p1 --outFileNamePrefix tmp_$tag --outSAMtype SAM --outSAMunmapped Within > log/$tag.log 2>&1 && mv tmp_${tag}Aligned.out.sam tmp_${tag}.sam");} #241204added
