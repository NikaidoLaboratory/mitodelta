...
my $usage = "usage: perl mitopoint.pl <config_file> <fastq file> <study name>";
die "Configuration file missing, $usage" unless $config_file;
die "Read file missing, $usage" unless $p1;
...
my $star = $User_Preferences->{star}; #241218added
my $lastal = $User_Preferences->{lastal};
my $lastsp = $User_Preferences->{lastsp};
my $blastn = $User_Preferences->{blastn};
my $blasttomaf = $User_Preferences->{blasttomaf};
my $python = $User_Preferences->{python};
...
my $seqtk = $User_Preferences->{seqtk};

#DATABASES
my $hsindex = $User_Preferences->{hsindex};
my $stindex = $User_Preferences->{stindex}; #241218added
my $faindex = $User_Preferences->{faindex};
my $lastindex = $User_Preferences->{lastindex};
my $blastdb = $User_Preferences->{blastdb};
...
if($nu_mt eq 'yes' && $enriched eq 'no'){
  ...
  if($dna eq 'yes'){system("$hisat2 -p $threads -x $hsindex --no-temp-splicesite --no-spliced-alignment --max-intronlen 5000 -U $p1 -S tmp_$tag.sam 2>> log/$tag.log");}
  if($dna eq 'no'){system("$star --runThreadN $threads --genomeDir $stindex --readFilesIn $p1 --outFileNamePrefix tmp_$tag --outSAMtype SAM --outSAMunmapped Within > log/$tag.log 2>&1 && mv tmp_${tag}Aligned.out.sam tmp_${tag}.sam");}  #241218added
  ...
  system("$samtools view -u -@ $threads -f 4 tmp_$tag.bam > tmp1_$tag.bam"); #flag to extract unmapped reads
...
if($nu_mt eq 'yes' && $o_mt eq 'yes' && $enriched eq 'no'){
  #REMAP ON MT GENOME, 241208changed(to blast)
  print scalar(localtime).": Map to MT genome\n";
  #system("$lastal -Q1 -e80 -P$threads $lastindex tmp_$tag.fq|$lastsp > tmp_$tag.maf");
  system("$seqtk seq -A tmp_$tag.fq > tmp_$tag.fa");
  system("$blastn -task blastn -query tmp_$tag.fa -db $blastdb -out tmp_$tag.blastn.out -outfmt \"6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qseq sseq qlen slen\" -num_threads $threads");
  system("$python $blasttomaf tmp_$tag.blastn.out tmp_$tag.maf");
...
if($nu_mt eq 'no' && $o_mt eq 'yes' && $enriched eq 'yes'){
  #REMAP ON MT GENOME
  print scalar(localtime).": Map to MT genome\n";
  #system("$reformat in=$p1 in2=$p2 out=tmp_$tag.fq overwrite=true addslash=t trimreaddescription=t spaceslash=f -Xmx100g &>> log/$tag.log");
  system("$lastal -Q1 -e80 -P$threads $lastindex $p1|$lastsp > tmp_$tag.maf");
  system("$mfcv sam -d tmp_$tag.maf|$samtools view -@ $threads -bt $mtfaindex -|$samtools sort -@ $threads -o bam/$tag.bam -");
  system("$samtools index bam/$tag.bam");
  system("$mfcv tab tmp_$tag.maf > tab/$tag.tab");
