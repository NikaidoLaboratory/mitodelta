...
my $bwa = $User_Preferences->{bwa}; #241218added
...
my $bwindex = $User_Preferences->{bwindex}; #241218added
...
if($nu_mt eq 'yes' && $enriched eq 'no'){
  ...
  if($dna eq 'no'){system("$bwa mem -t $threads $bwindex $p1 > tmp_$tag.sam 2>> log/$tag.log");}  #241218added
