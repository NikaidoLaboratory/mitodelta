...
#$ -notify
#$ -N mito.bl
#$ -o log/bwa.last.o.txt
#$ -e log/bwa.last.e.txt
#$ -pe threads 4
...
for ref in whole mt; do
    for back in breakspan spotdepth; do
        script="${input}/MitoSAlt_SE1.1.1_BWA_last_${back}.pl"
        config="${input}/config_human_${ref}_last.txt"
        for i in 7 8 9; do
            pt=$((i - 6))
            for seed in 1 2 3 4 5; do
                for size in "${sizes[@]}"; do
                    perl $script $config "$fastq/SRR1443333${i}_${size}_s${seed}.R1.fastq" "pt${pt}.${size}.s${seed}.${size}.s${seed}"
                done
                rm -f core.*
            done
        done
        mv indel "indel.BWA.${ref}.last.${back}"
        mkdir -p indel
    done
done
